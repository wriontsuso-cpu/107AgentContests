...
- 当前快照 1295 条记录中，1294 条通过标题、URL 与协议安全校验。
...
```text
POST {VITE_API_BASE_URL}/api/assistant/chat
```
...
```json
{
  "reply": "...",
  "clarifications": ["..."],
  "resources": [{ "id": "...", "title": "...", "summary": "...", "category": "...", "url": "/resources/..." }],
  "clues": ["竞赛与实践"]
}
```

不要把密钥放进 `VITE_*` 变量；它们会被打包进浏览器代码。鉴权与模型密钥必须保留在服务端。
