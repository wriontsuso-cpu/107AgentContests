...
describe('ResourceDetailPage', () => {
  it('shows traceable resource information and the original destination', async () => {
    ...
    expect(await screen.findByRole('heading', { name: resource.title }, { timeout: 15_000 })).toBeInTheDocument()
  ...
  it('shows related resources without repeating the current item', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '或许你还需要' }, { timeout: 15_000 })).toBeInTheDocument()
  ...
  it('handles an unknown id without crashing', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '这条资源暂时找不到' }, { timeout: 15_000 })).toBeInTheDocument()
