...
describe('HomePage', () => {
  it('keeps the homepage focused on the product and team story', () => {
    ...
    const heroHeading = screen.getByRole('heading', { name: '今天，想在科大做点什么？' })
    expect(within(heroHeading).getAllByTestId('headline-line').map((line) => line.textContent)).toEqual(['今天，想在科大', '做点什么？'])
    ...
    expect(within(websiteHeading).getAllByTestId('headline-line').map((line) => line.textContent)).toEqual(['在科大，找入口', '不必绕远路'])
    ...
    expect(screen.getByRole('heading', { name: '我们是，啊对对队' })).toHaveClass('headline-single-line')
    expect(screen.getByText(/余伊健、朱荣骐、陈泰然、赵世斌/)).toBeInTheDocument()
    expect(screen.queryByText(/雪后石碑/)).not.toBeInTheDocument()
