...
## 数据现状（2026-09-02 · 受限网址专项复检与清理）

- 依据团队整理的 238 条「受限网址」清单逐条复审，对目标行抽真实浏览器复核，避免把反爬 403 误判为失效（Autodesk 浏览器可正常访问，Canva 已切换国内教育版链接）
- 处置结果：70 条同一页面既无信息资讯、也不跳统一身份认证的 nginx 403 死页经用户指令删除；4 条换链修复；39 条由上轮 blocked 改判 reachable；125 条维持 blocked 登录墙
- 主库总量 **12882 → 12812**；受影响栏目：教务服务 -30、教务通知 -20、会议/学术交流 -20，其余栏目不变
- 三条 fixed 样例：课程目录直达 catalog.ustc.edu.cn；107 杯大赛与杨卫报告会通知换用中国科大新闻网主站稿；Canva 教育版使用 canva.cn/education
- 原文件备份：crawler 与发布仓两侧 `备份/受限网址复检_20260902_基线/`（不入库）；报告见工作区 `运行/受限网址复检_20260902/`
## 合规边界
